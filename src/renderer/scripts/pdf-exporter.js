// ==========================================================
// Pediatric Drug Calculator - PDF Exporter
// Clean and modular export system for Emergency & PRRT meds
// ==========================================================
...
export default class PDFExporter {
  ...
  static addPatientInfo(pdf, margin, yPosition, contentWidth, weight) {
    ...
    pdf.setFont('helvetica', 'normal');
    pdf.text(`${weight} kg`, margin + 25, yPosition);

    // Generated time
    pdf.setFont('helvetica', 'bold');
    pdf.text('Generated:', margin + 80, yPosition);
    pdf.setFont('helvetica', 'normal');
    pdf.text(new Date().toLocaleString(), margin + 115, yPosition);
  ...
  static addSummarySection(pdf, margin, yPosition, contentWidth, totalMeds, emergencyCount, prrtCount) {
    ...
    pdf.text(
      'All doses calculated per standard pediatric guidelines',
  ...
  static addMedicationSection(pdf, sectionTitle, medications, margin, yPosition, contentWidth, pageWidth) {
    ...
    pdf.setFillColor(isEmergency ? 231 : 52, isEmergency ? 76 : 152, isEmergency ? 60 : 219);
    ...
    medications.forEach((med, i) => {
      ...
      let x = margin + 2;
      const medName = med.name.length > 35 ? med.name.substring(0, 35) + '…' : med.name;
  ...
  static addFooter(pdf, pageWidth, margin) {
    ...
    pdf.text(
      `Generated by Pediatric Drug Calculator - ${new Date().getFullYear()}`,
      pageWidth / 2,
      pageHeight - 5,
      { align: 'center' }
    );
